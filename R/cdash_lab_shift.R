...
clean_lb_cq <- function(lb_cq, included, ex){
  ...
  med <- ex %>% mutate(ptno = as.numeric(CLIENTID), drugtype = EX_TRT) %>%
          ...
          arrange(ptno, PERIOD)  # sort by ptno and then PERIOD

  temp1 <- left_join(temp1, med, by = c("ptno","PERIOD")) %>%
  ...
  if(nrow(baseprob) > 0 ) message("Warning: Check your BASEPROB data")
...
labshift <- function(lb_cq, included, ex, UA=F){
   ...
   # clean the lab data
  laborig <- clean_lb_cq(lb_cq, included, ex)
